...
const SIM_CLASS = "bg-gradient-to-r from-emerald-500/20 to-teal-500/20 !border-emerald-400 ring-4 ring-emerald-500/10 scale-105";
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isActive ? SIM_CLASS : cn("bg-card border-border hover:border-primary/40", isSelected && "border-primary"),
      )}
    >
      <div className="flex justify-between items-center mb-1">
        <span className="text-[9px] font-bold tracking-widest uppercase text-purple-600 dark:text-purple-400">{step.type}</span>
        <button
    ...
    >
      <div className="flex items-center justify-between pb-4 border-b border-border">
        <div>
          ...
          <h3 className="font-bold text-base text-foreground mt-0.5">{step.label}</h3>
        </div>
        <button
          onClick={onClose}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => handleUpdateStepConfig(step.id, "value", parseInt(e.target.value))}
                className="w-full rounded-xl bg-muted/50 border border-border px-4 py-2.5 text-sm font-semibold text-foreground focus:outline-none focus:border-primary transition-colors"
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) => handleUpdateStepConfig(step.id, "unit", e.target.value)}
                className="w-full rounded-xl bg-muted/50 border border-border px-4 py-2.5 text-sm font-semibold text-foreground focus:outline-none focus:border-primary transition-colors"
              >
          ...
          <div className="space-y-4 pt-2 border-t border-border">
            ...
            <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/15 space-y-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                Describe what should be in the email and Gemini will generate optimized, persuasive copy.
              </p>
              <textarea
                rows={4}
                placeholder="Tell Gemini who you're targeting, the core offer, and the tone…"
                value={step.config.aiPrompt ?? ""}
                onChange={(e) => handleUpdateStepConfig(step.id, "aiPrompt", e.target.value)}
              ...
              >
                {isAiGenerating
                  ...
                  : <><Sparkles className="w-4 h-4" /><span>Write Email with Gemini</span></>
                }
              </button>
            </div>
                ...
                <React.Fragment key={step.id}>
                  ...
                  <motion.div
                    ...
                    className={cn(
                      ...
                      isSimActive
                        ? SIM_CLASS
                        : cn("bg-card border-border hover:border-primary/40", selectedStep?.id === step.id && "border-primary ring-2 ring-primary/20"),
                    )}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className={cn("text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full", TYPE_BADGE[step.type])}>
                        {step.type}
                      </span>
                      {step.type !== "trigger" && (
                        <button
                        ...
                        {yesSteps.map((yStep, yIdx) => (
                          <React.Fragment key={yStep.id}>
                            <BranchNode
                              ...
                              onRemove={() => handleRemoveStep(yStep.id, yesSteps, setYesSteps)}
                            />
                            {yIdx < yesSteps.length - 1 && (
                              <div className="w-0.5 h-8 bg-border relative flex justify-center items-center">
                                <button
                                  onClick={() => { setAddingToBranch("yes"); setShowAddMenuIdx(yIdx); }}
                        ...
                        {noSteps.map((nStep, nIdx) => (
                          <React.Fragment key={nStep.id}>
                            <BranchNode
                              ...
                              onRemove={() => handleRemoveStep(nStep.id, noSteps, setNoSteps)}
                            />
                            {nIdx < noSteps.length - 1 && (
                              <div className="w-0.5 h-8 bg-border relative flex justify-center items-center">
                                <button
                                  onClick={() => { setAddingToBranch("no"); setShowAddMenuIdx(nIdx); }}
                                  className="absolute w-5 h-5 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-white transition-colors scale-90"
                                >
                                  <Plus className="w-3.5 h-3.5" />
                                </button>
