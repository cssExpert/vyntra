import {
  BadRequestException,
  ConflictException,
  Injectable,
  NotFoundException,
} from '@nestjs/common';
import { PrismaService } from '../prisma/prisma.service';
import { CreatePackageDto, UpdatePackageDto } from './dto/package.dto';

@Injectable()
export class PackagesService {
  constructor(private prisma: PrismaService) {}

  /** Shape packages with a flat `modules: string[]` of module keys for the API. */
  private serialize(pkg: any) {
    return {
      ...pkg,
      modules: (pkg.modules ?? []).map((pm: any) => pm.module.key),
    };
  }

  private readonly include = { modules: { include: { module: true } } };

  async findAll() {
    const pkgs = await this.prisma.package.findMany({
      include: this.include,
      orderBy: { priceCents: 'asc' },
    });
    return pkgs.map((p) => this.serialize(p));
  }

  /** Public catalog for future self-signup. */
  async findPublic() {
    const pkgs = await this.prisma.package.findMany({
      where: { isActive: true, isPublic: true },
      include: this.include,
      orderBy: { priceCents: 'asc' },
    });
    return pkgs.map((p) => this.serialize(p));
  }

  async findOne(id: string) {
    const pkg = await this.prisma.package.findUnique({
      where: { id },
      include: this.include,
    });
    if (!pkg) throw new NotFoundException('Package not found');
    return this.serialize(pkg);
  }

  async create(dto: CreatePackageDto) {
    const slug = await this.uniqueSlug(dto.name);
    const moduleIds = await this.resolveModuleIds(dto.moduleKeys);

    const pkg = await this.prisma.package.create({
      data: {
        name: dto.name,
        slug,
        description: dto.description,
        priceCents: dto.priceCents ?? 0,
        billingCycle: dto.billingCycle,
        maxUsers: dto.maxUsers ?? 5,
        isActive: dto.isActive ?? true,
        isPublic: dto.isPublic ?? true,
        modules: { create: moduleIds.map((moduleId) => ({ moduleId })) },
      },
      include: this.include,
    });
    return this.serialize(pkg);
  }

  async update(id: string, dto: UpdatePackageDto) {
    await this.findOne(id);

    // Replace module assignments only when moduleKeys was provided.
    const moduleIds =
      dto.moduleKeys !== undefined
        ? await this.resolveModuleIds(dto.moduleKeys)
        : undefined;

    const pkg = await this.prisma.$transaction(async (tx) => {
      if (moduleIds !== undefined) {
        await tx.packageModule.deleteMany({ where: { packageId: id } });
      }
      return tx.package.update({
        where: { id },
        data: {
          name: dto.name,
          description: dto.description,
          priceCents: dto.priceCents,
          billingCycle: dto.billingCycle,
          maxUsers: dto.maxUsers,
          isActive: dto.isActive,
          isPublic: dto.isPublic,
          ...(moduleIds !== undefined && {
            modules: { create: moduleIds.map((moduleId) => ({ moduleId })) },
          }),
        },
        include: this.include,
      });
    });
    return this.serialize(pkg);
  }

  async remove(id: string) {
    await this.findOne(id);
    const inUse = await this.prisma.subscription.count({
      where: { packageId: id },
    });
    if (inUse > 0) {
      throw new ConflictException(
        'Cannot delete a package that organizations are subscribed to',
      );
    }
    await this.prisma.package.delete({ where: { id } });
    return { success: true };
  }

  // ── helpers ──────────────────────────────────────────────

  private async resolveModuleIds(keys?: string[]): Promise<string[]> {
    if (!keys || keys.length === 0) return [];
    const normalized = [...new Set(keys.map((k) => k.trim().toUpperCase()))];
    const modules = await this.prisma.module.findMany({
      where: { key: { in: normalized } },
    });
    if (modules.length !== normalized.length) {
      const found = new Set(modules.map((m) => m.key));
      const missing = normalized.filter((k) => !found.has(k));
      throw new BadRequestException(`Unknown module(s): ${missing.join(', ')}`);
    }
    return modules.map((m) => m.id);
  }

  private async uniqueSlug(name: string): Promise<string> {
    const base =
      name
        .toLowerCase()
        .trim()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
        .slice(0, 40) || 'package';
    let slug = base;
    let n = 1;
    while (await this.prisma.package.findUnique({ where: { slug } })) {
      slug = `${base}-${n++}`;
    }
    return slug;
  }
}
