...
const inputCls = 'w-full rounded-md px-2 py-1.5 text-xs focus:outline-none transition-colors bg-muted dark:bg-muted border border-border dark:border-border text-foreground dark:text-foreground focus:border-primary dark:focus:border-primary'
...
function SelectControl({ label, value, options, onChange }: {
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="text-[10px] w-20 flex-shrink-0 text-muted-foreground dark:text-muted-foreground">{label}</label>
...
function SpacingControl({ prefix, label }: { prefix: 'p' | 'm'; label: string }) {
  ...
  return (
    <div>
      <label className="text-[10px] mb-2 block font-medium uppercase tracking-wider text-muted-foreground dark:text-muted-foreground">{label}</label>
      <div className="grid grid-cols-4 gap-1.5">
        {sides.map(({ key, label: sl }) => (
          <div key={key}>
            <div className="text-[10px] text-center mb-1 text-muted-foreground dark:text-muted-foreground">{sl}</div>
    ...
    <aside className="w-64 flex flex-col h-full overflow-hidden border-l bg-card border-border dark:border-border">
      ...
      <div className="flex-shrink-0 px-4 py-3 border-b border-border dark:border-border">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className="text-xs truncate flex-1 text-muted-foreground dark:text-muted-foreground">{node.type}</span>
        </div>
        <p className="text-[10px] font-mono mt-0.5 text-muted-foreground dark:text-muted-foreground">id: {node.id}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <Section title="Classes" icon={Code2}>
          <div className="flex flex-wrap gap-1 mb-2 min-h-[20px]">
            {classes.length === 0
              ? <span className="text-xs italic text-muted-foreground dark:text-muted-foreground">No classes applied</span>
              : classes.map((cls) => (
          ...
          <div>
            <label className="text-[10px] block mb-1 text-muted-foreground dark:text-muted-foreground">Full class string</label>
            <textarea
        ...
        <Section title="Element" icon={Code2} defaultOpen={false}>
          <div>
            <label className="text-[10px] mb-1.5 block text-muted-foreground dark:text-muted-foreground">HTML Tag</label>
          ...
          {node.content !== undefined && (
            <div>
              <label className="text-[10px] mb-1 block text-muted-foreground dark:text-muted-foreground">Content</label>
              <textarea
                value={node.content || ''}
        ...
        <Section title="Typography" icon={Type}>
          ...
          />
          <div className="flex items-center gap-2">
            <label className="text-[10px] w-20 flex-shrink-0 text-muted-foreground dark:text-muted-foreground">Color</label>
            <InspectorInput
        ...
        <Section title="Size" icon={PencilRuler} defaultOpen={false}>
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={prefix}>
                <label className="text-[10px] mb-1 block text-muted-foreground dark:text-muted-foreground">{label}</label>
                <InspectorInput
        ...
        <Section title="Borders" icon={Box} defaultOpen={false}>
          ...
          />
          <div className="flex items-center gap-2">
            <label className="text-[10px] w-20 flex-shrink-0 text-muted-foreground dark:text-muted-foreground">Border</label>
            <InspectorInput
        ...
        <Section title="Background" icon={Palette} defaultOpen={false}>
          ...
          <div className="grid grid-cols-6 gap-1.5">
            {['bg-foreground','bg-card','bg-muted','bg-blue-600','bg-primary','bg-purple-600','bg-red-500','bg-orange-500','bg-yellow-400','bg-green-500','bg-teal-500','bg-cyan-500'].map((bg) => (
              <button
                key={bg}
                onClick={() => {
