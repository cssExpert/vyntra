...
// ── Domain Tab (ORG_ADMIN can manage their own custom domain) ─────────────────
          ...
          <div className="mt-3 space-y-1">
            {dns.dnsRecords.map((r, i) => (
              <p key={i} className="text-[11px] text-muted-foreground">
                <span className={`font-semibold ${r.type === "A" ? "text-blue-500" : r.type === "CNAME" ? "text-purple-500" : "text-amber-600"}`}>
                  {r.type} ({r.name})
                </span>{" — "}{r.note}
              </p>
            ))}
...
export function SettingsView() {
  ...
  const [activeTab, setActiveTab] = useState<
    "branding" | "notifications" | "domain" | "security"
  >("branding");
