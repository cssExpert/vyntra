...
function blogStatus(b: CmsDashboardStats["recentBlogs"][number]) {
  if (b.published) return { label: "Published", cls: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" };
  if (b.publishedAt && new Date(b.publishedAt) > new Date())
    return { label: "Scheduled", cls: "bg-blue-500/10 text-blue-600 border-blue-500/20" };
  return { label: "Draft", cls: "bg-amber-500/10 text-amber-600 border-amber-500/20" };
}
...
export function CmsDashboard() {
  ...
  const publishRate = stats.totalBlogs > 0
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground tracking-tight">CMS Overview</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Content performance across blogs, pages &amp; media</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/cms/blogs/new"
            className="flex items-center gap-1.5 px-3.5 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-bold rounded-lg transition-all"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-2xl p-5 col-span-1 sm:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-foreground">Publish Rate</span>
            </div>
            <span className="text-xs font-bold text-muted-foreground">{publishRate}%</span>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Featured Posts</p>
            <p className="text-2xl font-extrabold text-foreground leading-tight">{stats.featured}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">pinned to homepage</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-2xl p-5">
          <SectionHeader
            ...
            action={
              <Link href="/cms/blogs" className="text-[10px] font-bold text-primary hover:underline">
                View all →
              </Link>
            }
          />
          ...
          {stats.recentBlogs.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {stats.recentBlogs.map((blog) => {
                ...
                return (
                  <div key={blog.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                    {/* Cover thumbnail */}
                    ...
                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border ${status.cls}`}>
                      {status.label}
                    </span>
                  </div>
                );
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <SectionHeader
              ...
              action={
                <Link href="/cms/blog-categories" className="text-[10px] font-bold text-primary hover:underline">
                  Manage →
                </Link>
              }
            />
            ...
            {stats.topCategories.length === 0 ? (
              <p className="text-[11px] text-muted-foreground py-4 text-center">No categories used yet.</p>
            ) : (
              <div className="space-y-2.5">
                {stats.topCategories.map((cat, i) => {
                  const pct = stats.totalBlogs > 0 ? Math.round((cat.count / stats.totalBlogs) * 100) : 0;
                  return (
                    <div key={cat.name}>
                      <div className="flex items-center justify-between text-[10px] mb-1">
                        <span className="font-semibold text-foreground truncate">{cat.name}</span>
                        <span className="text-muted-foreground ml-2 shrink-0">{cat.count} post{cat.count !== 1 ? "s" : ""}</span>
                      </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                { label: "Write New Blog Post", href: "/cms/blogs/new", icon: Plus, cls: "bg-primary/10 text-primary hover:bg-primary/20" },
                { label: "Manage Categories", href: "/cms/blog-categories", icon: FolderOpen, cls: "bg-muted hover:bg-muted/70 text-foreground" },
                { label: "Edit Pages", href: "/cms/pages", icon: Globe, cls: "bg-muted hover:bg-muted/70 text-foreground" },
                { label: "Media Gallery", href: "/cms/gallery", icon: ImageIcon, cls: "bg-muted hover:bg-muted/70 text-foreground" },
              ].map((action) => {
