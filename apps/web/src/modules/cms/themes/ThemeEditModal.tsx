...
export function ThemeEditModal({ theme, onClose, onSave }: ThemeEditModalProps) {
          ...
          >
            ...
            <div className="px-6 py-5 border-b border-border flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-base font-bold text-foreground">Edit Theme</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Update metadata for{" "}
                    <span className="font-semibold text-foreground">{theme.title}</span>
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5 no-scrollbar">
                  ...
                  <div className="flex gap-2">
                    {(["published", "draft"] as GalleryStatus[]).map((s) => (
                      <button
                        ...
                        onClick={() => setStatus(s)}
                        className={cn(
                          "flex-1 py-2.5 rounded-sm border text-xs font-bold transition-all flex items-center justify-center gap-1.5",
                          status === s
                            ? s === "published"
              ...
              <div>
                <label className={labelCls}>
                  Cover Image URL{" "}
                  <span className="normal-case font-normal text-muted-foreground/60">(leave blank to keep current)</span>
                </label>
                <input
                  type="url"
                  value={thumbnailUrl}
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/60 flex justify-end gap-3 shrink-0">
              ...
              <button
                type="button"
                onClick={handleSave}
                className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-sm text-sm font-semibold transition-all shadow-sm active:scale-[0.98]"
              >
                Save Changes
              </button>
            </div>
