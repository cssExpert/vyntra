        ...
        {theme.thumbnail ? (
          ...
          >
            <div className="flex gap-1.5">
              {[
                primary,
                getVar(v, "--secondary", "#64748b"),
                getVar(v, "--accent", "#0ea5e9"),
              ].map((c, i) => (
                <div
                  key={i}
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-0.5 text-[9px] font-medium bg-muted text-muted-foreground px-1.5 py-0.5 rounded border border-border/60"
                >
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          {livePreviewUrl ? (
            ...
            <button
              onClick={() => onPreview(theme)}
              className="text-xs text-muted-foreground hover:text-foreground px-2.5 py-1 rounded-lg border border-border hover:bg-muted transition-all"
            >
              Preview
            </button>
          )}
          <button
            onClick={() => onPreview(theme)}
            title={t("colordetails", { defaultValue: "Color details" })}
            className="p-1.5 text-muted-foreground hover:text-foreground border border-border rounded-lg hover:bg-muted transition-all"
          >
            <Info className="w-3.5 h-3.5" />
          </button>
          <button
...
export function ThemesView() {
  ...
  const processedThemes = useMemo(() => {
    ...
    if (selectedCategory !== "All") {
      result = result.filter(
        (t) => getCategory(t.variables) === selectedCategory,
      );
    }
    result.sort((a, b) => {
      if (sortBy === "oldest")
