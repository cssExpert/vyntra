...
import {
  Inbox, Send, FileText, Star, Trash2, Pencil,
} from "lucide-react";
...
export function MailSidebar({ activeFolder, setActiveFolder, emails, onCompose }: MailSidebarProps) {
  ...
  return (
    <aside className="w-56 shrink-0 flex flex-col gap-6 pr-4 border-r border-border">
      ...
      {/* Folder tabs — same spring-pill as the rest of the app */}
      ...
      <div>
        ...
        <div className="space-y-0.5">
          {LABELS.map((label) => (
            <div
              key={label.id}
              className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer"
            >
