...
import {
  Plus, Zap, Eye, Pencil, Trash2, Play, Pause,
  ShoppingCart, UserPlus, Package, Bell, RefreshCw,
} from "lucide-react";
      ...
      {!isLoaded ? (
        ...
        >
          <PageHeader
            ...
            breadcrumbs={[{ label: "Store", href: "/store" }, { label: "Automations" }]}
          >
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: "Paused",   value: rules.filter((r) => r.status === "paused").length,      color: "text-warning" },
              { label: "Total Runs", value: rules.reduce((s, r) => s + r.runCount, 0).toLocaleString(), color: "text-info" },
            ].map((s) => (
            ...
            {rules.map((rule) => (
              <motion.div
                ...
                className={cn(
                  ...
                  rule.status === "active" ? "border-border hover:border-primary/30" : "border-border opacity-60",
                )}
              >
                {/* Icon */}
                <div className={cn(
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold text-foreground text-[13px]">{rule.name}</p>
                    {rule.isBuiltIn && (
                      <span className="text-[10px] font-semibold bg-info/10 text-info px-1.5 py-0.5 rounded-full">Built-in</span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-0.5">{rule.description}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[11px] text-muted-foreground">
                      Trigger: <span className="text-foreground font-medium">{TRIGGER_LABELS[rule.trigger] ?? rule.trigger}</span>
                    </span>
                    <span className="text-[11px] text-muted-foreground">
                      Runs: <span className="text-foreground font-semibold tabular-nums">{rule.runCount.toLocaleString()}</span>
                    </span>
                    {rule.lastRunAt && (
                      <span className="text-[11px] text-muted-foreground">
                        Last: <span className="text-foreground">{new Date(rule.lastRunAt).toLocaleDateString()}</span>
                      </span>
                    )}
                  </div>
                </div>
                ...
                {/* Status badge */}
                <StatusBadge
                  variant={rule.status === "active" ? "success" : rule.status === "paused" ? "warning" : "muted"}
                ...
                >
                  {rule.status === "active" ? <Pause size={11} /> : <Play size={11} />}
