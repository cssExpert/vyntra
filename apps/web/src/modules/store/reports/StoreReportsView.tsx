...
export function StoreReportsView() {
  ...
  const ordersByStatus = [
    {
      label: "Delivered",
      count: SAMPLE_ORDERS.filter((o) => o.status === "delivered").length,
      color: "bg-success",
    },
    {
      label: "Processing",
      count: SAMPLE_ORDERS.filter((o) => o.status === "processing").length,
      color: "bg-info",
    },
    {
      label: "Pending",
      count: SAMPLE_ORDERS.filter((o) => o.status === "pending").length,
      color: "bg-warning",
    },
    {
      label: "Cancelled",
      count: SAMPLE_ORDERS.filter((o) => o.status === "cancelled").length,
      color: "bg-error",
    },
  ];
                ...
                return (
                  <div key={p.id} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-success tabular-nums shrink-0">
                      ${((p.totalSales ?? 0) * p.price).toFixed(0)}
                    </span>
                  </div>
                );
              })}
