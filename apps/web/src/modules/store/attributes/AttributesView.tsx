...
function getAttribute(): StoreAttribute[] {
  ...
  const deleted = JSON.parse(localStorage.getItem("store_attributes_deleted") || "[]");
      ...
      {!isLoaded ? (
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Total Attributes", value: getAttribute().length, color: "text-foreground" },
              { label: "Used In Variation", value: getAttribute().filter((a) => a.usedInVariation).length, color: "text-primary" },
              { label: "Specification Only", value: getAttribute().filter((a) => !a.usedInVariation).length, color: "text-muted-foreground" },
            ].map((s) => (
                  ...
                  {rows.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-16 text-center text-muted-foreground">
                        No attributes found.{" "}
                        <button onClick={() => router.push("/store/attributes/add")} className="text-primary underline cursor-pointer">Add one</button>
                      </td>
                    </tr>
                  ) : rows.map((attr) => (
                    <tr key={attr.id} className="group hover:bg-muted/40 transition-colors">
                      ...
                      <td className="py-4 px-4 text-muted-foreground text-[13px]">
                        {attr.attributeType === "color" ? "Color Picker" : FIELD_TYPE_LABELS[attr.fieldType]}
                      </td>
                      <td className="py-4 px-4">
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {attr.options.slice(0, 5).map((opt) => (
                            <span key={opt.id} className="flex items-center gap-1 text-[11px] bg-muted px-2 py-0.5 rounded-sm text-foreground">
                              {opt.colorHex && (
                                <span className="inline-block w-2.5 h-2.5 rounded-full border border-border" style={{ background: opt.colorHex }} />
                              )}
                              {opt.name}
                            </span>
                          ))}
                          {attr.options.length > 5 && (
                            <span className="text-[11px] text-muted-foreground px-1">+{attr.options.length - 5} more</span>
                          )}
                        </div>
                      </td>
                      <td className="py-4 px-4">
