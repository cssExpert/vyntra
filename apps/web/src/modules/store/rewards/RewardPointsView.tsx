...
function AdjustPointsModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pts || pts <= 0) { setError("Enter a positive number of points."); return; }
    if (!reason.trim())   { setError("Enter a reason."); return; }
    setSaving(true);
    setError("");
    try {
      await storeCustomers.addRewardPoints(reward.customerId, pts, reason.trim());
      onSaved();
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-foreground">Add Reward Points</h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              {reward.customerName} — Current: <strong>{reward.points.toLocaleString()} pts</strong>
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-foreground">Points to add</label>
            <input
              type="number"
              min="1"
              step="1"
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" radius="sm" className="flex-1" disabled={saving}>
              {saving ? <Loader2 size={14} className="animate-spin" /> : "Add Points"}
            </Button>
          </div>
        </form>
      </motion.div>
...
export function RewardPointsView() {
  ...
  const [adjustReward, setAdjustReward] = useState<CustomerReward | null>(null);
  ...
  return (
    <>
        ...
        {!isLoaded || isLoading ? (
          ...
          >
                          ...
                          <td className="py-4 px-4 text-right">
                            <TableActionMenu
                              items={[
                                ...
                                { label: t("adjust"),       icon: <Pencil size={14} />, onClick: () => setAdjustReward(r) },
                              ]}
                            />
                          </td>
                        </tr>
            ...
            <p className="text-xs text-muted-foreground text-right">
              Total points outstanding: <strong className="text-foreground">{totalPoints.toLocaleString()}</strong>
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {adjustReward && (
        <AdjustPointsModal
          reward={adjustReward}
          onClose={() => setAdjustReward(null)}
          onSaved={() => { setAdjustReward(null); load(); }}
        />
      )}
    </>
  );
