...
export function AddCouponView() {
  ...
  const handleSave = async () => {
    setError(null);
    if (!code.trim()) {
      setError(t("errorCodeRequired", { defaultValue: "Coupon code is required." }));
      return;
    }
    setIsSaving(true);
    try {
      await storeCoupons.create({
        code: code.trim().toUpperCase(),
        type,
        value: parseFloat(value) || 0,
        minimumSpend: parseFloat(minimumSpend) || undefined,
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key="content" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, ease: "easeOut" }} className="flex flex-col gap-4">
        <PageHeader
          title={t("addCoupon", { defaultValue: "Add Coupon" })}
          description={t("addCouponDescription", { defaultValue: "Create a new discount code for customers." })}
          breadcrumbs={[
            { label: t("store"), href: "/store" },
            { label: t("title"), href: "/store/coupons" },
            { label: t("addCoupon", { defaultValue: "Add Coupon" }) },
          ]}
        >
          <Button variant="outline" size="lg" onClick={() => router.push("/store/coupons")}>
            <X size={16} />
            {t("cancel")}
          </Button>
